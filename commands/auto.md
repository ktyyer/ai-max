...
输入你的需求，`/aimax:auto` 会自动完成：需求分析 → 复杂度评估 → 规划设计 → 编码实现 → 测试验证 → 代码审查 → Git 提交 → 经验沉淀。
...
`/aimax:auto` 是**超级命令**，内部自动调用所有能力。精细控制时可使用：

| 命令 | 用途 |
|------|------|
| `/aimax:plan` | 只规划不编码 |
| `/aimax:deep-plan` | 复杂重构/架构两阶段深度规划 |
| `/aimax:status` | 查看项目状态和 Self-* 学习情况 |
| `/aimax:help` | 查看所有可用命令 |

**推荐**：大多数情况下直接使用 `/aimax:auto` 即可。
...
```bash
/aimax:auto 用 Spring Boot 实现用户分页查询接口
/aimax:auto 在 React 项目中实现可复用的表单组件
/aimax:auto 审查最近的代码改动
...
```bash
/aimax:auto 写代码          # 太模糊
/aimax:auto 重构整个系统      # 太复杂，建议用 /aimax:deep-plan
```

---

**核心原则**：
1. **一个入口** — `/aimax:auto` 完成所有事情
