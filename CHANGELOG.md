...
- **新命令**
  ...
  - `/aimax:status` - 查看项目状态和已学习模式
  - `/aimax:help` - 显示帮助和使用示例

### Changed

- **精简命令**：15 个 → 5 个（-67%）
  - 保留：`/aimax:auto`, `/aimax:plan`
  - 新增：`/aimax:fix`, `/aimax:status`, `/aimax:help`
...
# 主要变化
- 14 个命令已整合到 /aimax:auto
- 现在只需记住 /aimax:auto 一个命令
- 其他命令自动决策，无需手动调用
```
